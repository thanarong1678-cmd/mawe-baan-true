...
function ensureStockStyles() {
  ...
  style.textContent = `
    ...
    .mawe-stock-date{grid-column:1 / span 2;display:flex;align-items:center;gap:5px;font-size:9px;font-weight:800;color:#a56737;margin-top:2px}.mawe-stock-date input{width:94px;height:23px;border:1px solid #f3c77e;border-radius:7px;background:#fff8ed;color:#8a5428;font-size:9px;padding:2px 4px;font-weight:800;cursor:pointer}
    ...
    .mawe-header-quote .mawe-arrow{font-size:29px;color:#fff;text-shadow:0 2px 3px rgba(124,45,18,.12);line-height:1;flex:0 0 auto;cursor:pointer;user-select:none}.mawe-header-quote .mawe-arrow:hover{transform:scale(1.18)}.mawe-header-quote .mawe-arrow:active{transform:scale(.92)}
    .mawe-header-quote-text{flex:1;min-width:0;text-align:center;white-space:nowrap;overflow-x:auto;overflow-y:hidden;text-overflow:clip;scrollbar-width:none;touch-action:pan-x}.mawe-header-quote-text::-webkit-scrollbar{display:none}
    ...
    @media(max-width:767px){
      header{display:flex!important;flex-direction:column!important;gap:10px!important;padding:14px!important}header>div:first-child{width:100%!important;display:flex!important;align-items:center!important;gap:10px!important}
      header>div:first-child>div:first-child{width:68px!important;height:68px!important;min-width:68px!important;min-height:68px!important;flex:0 0 68px!important;font-size:34px!important;border-width:4px!important;aspect-ratio:1/1!important}.mawe-cat-face{width:36px!important;height:36px!important;font-size:36px!important;flex-basis:36px!important}
      header>div:first-child>div:nth-child(2){max-width:calc(100% - 78px)!important}header h1{font-size:26px!important}header h1+button{font-size:11px!important;padding:6px 10px!important}.mawe-header-quote{width:100%!important;max-width:none!important;font-size:11px!important;padding:9px 11px!important}.mawe-header-quote .mawe-arrow{font-size:23px}
      .mawe-header-stock{position:static!important;transform:none!important;width:100%!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;order:2!important}.mawe-stock-card{width:auto!important;min-height:82px!important;padding:9px!important;border-radius:16px!important;grid-template-columns:36px 1fr!important;gap:3px 6px!important}.mawe-stock-icon{width:36px;height:36px;font-size:22px}.mawe-stock-label{font-size:11px}.mawe-stock-value{font-size:10px}.mawe-stock-value strong{font-size:21px}.mawe-stock-owner{font-size:8px}.mawe-stock-controls button{width:25px;height:22px}.mawe-stock-date{font-size:8px}.mawe-stock-date input{width:86px;height:21px;font-size:8px}.mawe-header-sleepy,.mawe-header-note,.mawe-header-paws{display:none!important}
    }
    @media(min-width:768px) and (max-width:1100px){
      header{display:flex!important;flex-direction:column!important;gap:12px!important}header>div:first-child{width:100%!important}header>div:first-child>div:first-child{width:92px!important;height:92px!important;min-width:92px!important;min-height:92px!important;flex:0 0 92px!important;font-size:46px!important;aspect-ratio:1/1!important}.mawe-cat-face{width:46px!important;height:46px!important;font-size:46px!important;flex-basis:46px!important}header>div:first-child>div:nth-child(2){max-width:calc(100% - 104px)!important}header h1{font-size:34px!important}header h1+button{font-size:13px!important;padding:7px 13px!important}.mawe-header-quote{width:100%!important;max-width:none!important;font-size:13px!important}.mawe-header-stock{position:static!important;transform:none!important;width:100%!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;order:2!important}.mawe-stock-card{width:auto!important;min-height:96px!important}.mawe-header-sleepy,.mawe-header-note,.mawe-header-paws{display:none!important}
    }
  ...
  const updateHeader = useCallback(async () => {
    ...
    if (oldBubble) {
      ...
      oldBubble.querySelector('[data-dir="prev"]')?.addEventListener('click', (e) => {
        e.stopPropagation(); const current = Number(oldBubble!.dataset.quoteIndex || 0); const next = (current - 1 + HEADER_QUOTES.length) % HEADER_QUOTES.length; oldBubble!.dataset.quoteIndex = String(next); if (q) q.textContent = HEADER_QUOTES[next]
      })
      oldBubble.querySelector('[data-dir="next"]')?.addEventListener('click', (e) => {
        e.stopPropagation(); const current = Number(oldBubble!.dataset.quoteIndex || 0); const next = (current + 1) % HEADER_QUOTES.length; oldBubble!.dataset.quoteIndex = String(next); if (q) q.textContent = HEADER_QUOTES[next]
      })
    }
    ...
    if (user) {
      ...
      if (litter) { litterId=litter.id; bags=Number(litter.bags_left)||0; boxes=Number(litter.box_count)||0; changedDate=litter.last_changed_date || litter.last_changed || '' }
    }
    const saveStock = async (nextBags:number,nextBoxes:number) => {
      if (!user) return
      if (litterId) await supabase.from('cat_litter').update({bags_left:Math.max(0,nextBags),box_count:Math.max(0,nextBoxes)}).eq('id',litterId)
      else { const today=new Date().toISOString().slice(0,10); const {data}=await supabase.from('cat_litter').insert({bags_left:Math.max(0,nextBags),box_count:Math.max(0,nextBoxes),last_changed:today,last_changed_date:today,user_id:user.id}).select('id').maybeSingle(); if(data){litterId=data.id;changedDate=today} }
    }
    const saveChangedDate = async (value:string) => {
      if (!user || !value) return
      changedDate=value
      if (litterId) await supabase.from('cat_litter').update({last_changed_date:value,last_changed:value}).eq('id',litterId)
      else { const {data}=await supabase.from('cat_litter').insert({bags_left:bags,box_count:boxes,last_changed:value,last_changed_date:value,user_id:user.id}).select('id').maybeSingle(); if(data) litterId=data.id }
    }
    stockWrap.innerHTML=`<div class="mawe-stock-card"><div class="mawe-stock-icon">🛍️</div><div><div class="mawe-stock-label">ทรายแมว</div><div class="mawe-stock-value"><strong>${bags}</strong> ถุง</div><div class="mawe-stock-owner">🐱 ของ${catName}</div></div><div class="mawe-stock-controls"><button data-stock="bm">−</button><button data-stock="bp">+</button></div></div><div class="mawe-stock-card"><div class="mawe-stock-icon">🧺</div><div><div class="mawe-stock-label">กระบะทราย</div><div class="mawe-stock-value"><strong>${boxes}</strong>/3</div><div class="mawe-stock-owner">🐱 ของ${catName}</div></div><div class="mawe-stock-controls"><button data-stock="xm">−</button><button data-stock="xp">+</button></div><div class="mawe-stock-date"><span>🧹 เปลี่ยน:</span><input type="date" value="${changedDate}" aria-label="วันที่เปลี่ยนกระบะทรายล่าสุด" /></div></div>`
